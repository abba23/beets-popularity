...
class Popularity(BeetsPlugin):
	...
	def commands(self):
		...
		command.parser.add_option(
            '-n', '--nowrite', action='store_true', dest='nowrite',
            default=False, help='print the popularity values without storing them')
		return [command]
	...
	def _set_popularity(self, item, nowrite):
		...
		try:
			...
			self._log.info(u'{0.artist} - {0.album} - {0.title}: {1}', item, popularity)
			...
			self._log.debug(u'{0.title} - {0.artist} not found on Spotify', item)
